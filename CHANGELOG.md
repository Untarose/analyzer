...
---
## [0.1.6-dev] - 2025-07-23
### 🐛 修正（Fixed）
- 【Analyzer】delete_groupメソッドのエラーハンドリングを改善し、グループが存在しない場合にメッセージを表示するように変更
- 【ExecContext】ExecContextの検証に成功した際のメッセージを追加
- 【Executor】_is_expext_resultメソッドのエラーメッセージを改善し、関数を実行するよう促すメッセージを追加
---
## [0.1.5-dev] - 2025-07-23
### 🐛 修正（Fixed）
- 【README】README.mdを更新し、データ解析操作基盤の概要、コア概念、使い方を詳細に記述
---
## [0.1.4-dev] - 2025-07-13
### 🚀 追加（Added）
- 【Analyzer】指定したgroupの情報を表示するprintメソッドを追加．引数に指定しないとすべて表示．
    - printメソッド
    - 内部用 _print_singleメソッド
        - 一つのグループの情報を表示するメソッドで，print内で複数使用される
---
...
- 【Analyzer】master読み込み時，nameが○○_rate, ○○_waveform, と読み込まれるはずが，rate, waveformとなってしまった原因を修正
    - _load_groups_from_master内の，_load_units呼び出しの際の引数を適切なものに変更
---
## [0.1.1-dev] - 2025-07-13
### 🛠 変更（Changed）
- src/cores, src/interfaces ,tests内のディレクトリ構造を変更
    - src/cores, src/interfaces ,tests
        - analyzer
        - builder
        - data_unit
        - data_group
